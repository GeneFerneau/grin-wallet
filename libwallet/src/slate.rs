...
use crate::grin_keychain::{
	BlindSum, BlindingFactor, ExtKeychain, Identifier, Keychain, SwitchCommitmentType,
};
...
impl Slate {
		...
		K: Keychain,
	{
		...
		let part_sig = aggsig::calculate_partial_sig(
			...
			&context.sec_key,
			&context.sec_nonce,
			context.get_secret_atomic_nonce(),
		...
		K: Keychain,
	{
		...
		aggsig::verify_partial_sig(
			...
			part_sig,
			&nonce_sum,
			self.participant_data[opdata_idx].atomic_nonce.as_ref(),
		...
		K: Keychain,
	{
		...
		let atomic_nonce = context.get_public_atomic_nonce(keychain.secp())?;
