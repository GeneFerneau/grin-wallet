	...
	K: Keychain + 'a,
{
	...
	// fill public keys
	slate.fill_round_1(&keychain, &mut context)?;
	// Create partial signature using the secret atomic nonce,
	// allows sender to recover the atomic nonce when the
...
/// Recover atomic nonce from final signature and adaptor signature
pub fn recover_atomic_nonce<'a, T: ?Sized, C, K>(
	wallet: &mut T,
	keychain_mask: Option<&SecretKey>,
	slate: &Slate,
	tx_kernel: &TxKernel,
	...
	K: Keychain + 'a,
{
	...
	// calculate sr' - sr to recover the atomic nonce
