...
fn test_atomic_swap_multisig_tx() {
	...
	let atomic_nonce = SecretKey::from_slice(receiver_keychain.secp(), &[2; 32]).unwrap();
	receiver_ctx.set_atomic_nonce(atomic_nonce.clone());
	...
	//sr.add_assign(secp, &atomic_nonce).unwrap();
	sr.neg_assign(secp).unwrap();
	srp.add_assign(secp, &sr).unwrap();
	assert_eq!(srp, atomic_nonce);
