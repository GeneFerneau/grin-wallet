...
/// Recover the atomic nonce from an adaptor signature and kernel excess signature
pub fn recover_atomic_nonce<L, C, K>(
	owner_api: &mut Owner<L, C, K>,
	keychain_mask: Option<&SecretKey>,
	...
	K: keychain::Keychain + 'static,
{
	...
	controller::owner_single_use(None, keychain_mask, Some(owner_api), |api, m| {
		let result = api.recover_atomic_nonce(m, &slate);
		match result {
			Ok(_) => {
...
pub fn get_atomic_nonces<L, C, K>(
	owner_api: &mut Owner<L, C, K>,
	keychain_mask: Option<&SecretKey>,
	args: GetAtomicNoncesArgs,
) -> Result<(), Error>
where
	L: WalletLCProvider<'static, C, K> + 'static,
	C: NodeClient + 'static,
